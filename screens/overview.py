...
class BudgetProgressItem(BoxLayout):
    ...
    b_warning = StringProperty("")   # PERINGATAN BARU
...
class OverviewContent(BoxLayout):
    def update_overview(self):
        ...
        for item in data:
            ...
            # ===== LOGIKA WARNA BARU =====
            if p >= 1.0:
                # Progress penuh → HIJAU
                color = [0.2, 0.8, 0.3, 1]
                p_display = 1.0
                warning = "Budget sudah terpenuhi atau melebihi"

            elif p > 0:
                # Ada progress tapi belum full → KUNING
                color = [0.95, 0.8, 0.2, 1]
                p_display = p
                warning = ""

            else:
                # Tidak ada progress → TIDAK ADA WARNA
